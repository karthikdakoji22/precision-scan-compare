...
export const STLViewer: React.FC<STLViewerProps> = ({
  ...
  const loadSTLFile = (file: File): Promise<THREE.BufferGeometry> => {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = e => {
        try {
          const geom = stlLoaderRef.current!.parse(e.target!.result as ArrayBuffer);
          geom.computeVertexNormals();
          resolve(geom);
        } catch (err) { reject(err); }
      };
      reader.onerror = () => reject(new Error('Failed to load STL'));
      reader.readAsArrayBuffer(file);
    });
  };
  ...
  const computeDeviation = (
    queryGeom: THREE.BufferGeometry,
    refGeom: THREE.BufferGeometry
  ) => {
    const qPos = queryGeom.attributes.position.array as Float32Array;
    const rPos = refGeom.attributes.position.array as Float32Array;
    const deviations: number[] = [];
    for (let i = 0; i < qPos.length; i += 3) {
      const qv = new THREE.Vector3(qPos[i], qPos[i + 1], qPos[i + 2]);
      let minDist = Infinity;
      for (let j = 0; j < rPos.length; j += 3) {
        const rv = new THREE.Vector3(rPos[j], rPos[j + 1], rPos[j + 2]);
        const d = qv.distanceTo(rv);
    ...
    const stats = {
      ...
      average: deviations.reduce((s, v) => s + v, 0) / deviations.length,
  ...
  // Color mapping: Mint green (matched) / Pink (mismatched)
  const makeColoredMesh = (queryGeom: THREE.BufferGeometry, refGeom: THREE.BufferGeometry) => {
    const deviations = computeDeviation(queryGeom, refGeom);
    const colors = new Float32Array(queryGeom.attributes.position.count * 3);
    ...
    for (let i = 0; i < deviations.length; i++) {
      ...
      if (d <= threshold) {
        // Mint green
        colors[idx] = 152 / 255;
        colors[idx + 1] = 251 / 255;
        colors[idx + 2] = 152 / 255;
      } else {
        // Pink
        colors[idx] = 255 / 255;
        colors[idx + 1] = 105 / 255;
        colors[idx + 2] = 180 / 255;
      }
    }
    queryGeom.setAttribute('color', new THREE.BufferAttribute(colors, 3));
    return new THREE.Mesh(
      queryGeom,
      new THREE.MeshPhongMaterial({ vertexColors: true, side: THREE.DoubleSide, wireframe: showWireframe })
    );
  };
  ...
  useEffect(() => {
    ...
    const run = async () => {
      setLoadingSTL(true);
      // Clear old
      sceneRef.current!.children = sceneRef.current!.children.filter(c => !(c as any).isSTLModel);

      let refGeom: THREE.BufferGeometry | null = null;
      let queryGeom: THREE.BufferGeometry | null = null;

      if (referenceFile) refGeom = await loadSTLFile(referenceFile);
      if (queryFile) queryGeom = await loadSTLFile(queryFile);

      if (analysisComplete && refGeom && queryGeom) {
        const mesh = makeColoredMesh(queryGeom, refGeom);
        (mesh as any).isSTLModel = true;
        sceneRef.current!.add(mesh);
        loadedMeshesRef.current.superimposed = mesh;
      } else {
        if (refGeom) {
          const m = new THREE.Mesh(refGeom, new THREE.MeshLambertMaterial({ color: 0x22c55e, wireframe: showWireframe }));
          (m as any).isSTLModel = true; m.visible = showReference;
          sceneRef.current!.add(m);
          loadedMeshesRef.current.reference = m;
        }
        if (queryGeom) {
          const m = new THREE.Mesh(queryGeom, new THREE.MeshLambertMaterial({ color: 0x3b82f6, wireframe: showWireframe }));
          (m as any).isSTLModel = true; m.visible = showQuery;
          sceneRef.current!.add(m);
          loadedMeshesRef.current.query = m;
        }
      }
      setLoadingSTL(false);
    };
    run();
  }, [referenceFile, queryFile, analysisComplete, showReference, showQuery]);
  ...
  return (
    <Card className="h-full flex flex-col">
      {/* Header */}
      <div className="p-6 border-b border-border bg-gradient-to-r from-surface to-background">
        <h3 className="text-xl font-semibold text-foreground mb-1">3D Superimposition Analysis</h3>
        {deviationStats && analysisComplete && (
          <p className="text-sm text-muted-foreground">
            Min: {deviationStats.min.toFixed(4)}mm • Max: {deviationStats.max.toFixed(4)}mm • Avg: {deviationStats.average.toFixed(4)}mm
          </p>
        )}
      </div>
